---
name: I_CACLEARINGCATEGORY
description: Caclearingcategory
semantic_en: "clearing category master data — identifies categories used to organize clearing activities."
semantic_vi: "dữ liệu chính danh mục thanh toán — xác định các danh mục dùng để tổ chức các hoạt động thanh toán."
keywords:
  - danh mục thanh toán
  - clearing
  - master data
app_component: FI-CA-2CL
software_component: SAPSCORE
release_state: released
clean_core_level: A
system_type: public_cloud
source_available: true
tags:
  - FI
  - FI-CA
  - interface-view
  - component:FI-CA-2CL
  - lob:Finance
---
# I_CACLEARINGCATEGORY

**Caclearingcategory**

| Property | Value |
|---|---|
| App Component | `FI-CA-2CL` |
| Software Component | `SAPSCORE` |
| Release State | Released (Level A) |
| System Type | S/4HANA Cloud Public Edition |

## Fields

| Field | Data Source |
|---|---|
| `CAClearingCategory` | `vertyp` |
| `_Text` | *Association* |

## Associations

| Alias | Target View | Cardinality |
|---|---|---|
| `_Text` | `I_CAClearingCategoryText` | [0..*] |

## Source Code

```abap
@AccessControl.authorizationCheck: #NOT_REQUIRED

@EndUserText.label: 'Clearing Category'

@Metadata.ignorePropagatedAnnotations: true

@ObjectModel: { modelingPattern: #NONE,
                representativeKey: 'CAClearingCategory',
                sapObjectNodeType.name: 'ContrAcctgClearingCategory',
                supportedCapabilities: [ #CDS_MODELING_ASSOCIATION_TARGET, 
                                         #CDS_MODELING_DATA_SOURCE,
                                         #SQL_DATA_SOURCE ],
                usageType: { dataClass: #CUSTOMIZING,
                             serviceQuality: #A,
                             sizeCategory: #S } }

@VDM.viewType: #BASIC

define view entity I_CAClearingCategory
  as select from tfk111
  
  association [0..*] to I_CAClearingCategoryText as _Text on $projection.CAClearingCategory = _Text.CAClearingCategory

{
      @ObjectModel.text.association: '_Text'
  key vertyp as CAClearingCategory,

      _Text
}
```
