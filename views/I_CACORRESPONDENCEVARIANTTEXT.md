---
name: I_CACORRESPONDENCEVARIANTTEXT
description: Cacorrespondencevarianttext
semantic_en: "correspondence variant texts — provides multilingual names for correspondence variants."
semantic_vi: "văn bản biến thể thư từ — cung cấp tên đa ngôn ngữ cho các biến thể thư từ."
keywords:
  - biến thể thư từ
  - tên
  - mô tả
app_component: FI-CA-2CL
software_component: SAPSCORE
release_state: released
clean_core_level: A
system_type: public_cloud
source_available: true
tags:
  - FI
  - FI-CA
  - interface-view
  - text-view
  - text
  - component:FI-CA-2CL
  - lob:Finance
---
# I_CACORRESPONDENCEVARIANTTEXT

**Cacorrespondencevarianttext**

| Property | Value |
|---|---|
| App Component | `FI-CA-2CL` |
| Software Component | `SAPSCORE` |
| Release State | Released (Level A) |
| System Type | S/4HANA Cloud Public Edition |

## Fields

| Field | Data Source |
|---|---|
| `CACorrespondenceVariant` | `coprc` |
| `Language` | `spras` |
| `CACorrespondenceVariantName` | `cotxt` |
| `_Language` | *Association* |

## Associations

| Alias | Target View | Cardinality |
|---|---|---|
| `_Language` | `I_Language` | [0..1] |

## Source Code

```abap
@AccessControl.authorizationCheck: #NOT_REQUIRED

@Analytics.technicalName: 'ICACORRVARTXT'

@EndUserText.label: 'Correspondence Variant - Text'

@Metadata.ignorePropagatedAnnotations: true

@ObjectModel: { dataCategory: #TEXT,
                modelingPattern: #LANGUAGE_DEPENDENT_TEXT,
                representativeKey: 'CACorrespondenceVariant',
                supportedCapabilities: [ #CDS_MODELING_ASSOCIATION_TARGET,
                                         #CDS_MODELING_DATA_SOURCE,
                                         #LANGUAGE_DEPENDENT_TEXT,
                                         #SEARCHABLE_ENTITY,
                                         #SQL_DATA_SOURCE ],
                usageType: { dataClass: #CUSTOMIZING,
                             serviceQuality: #A,
                             sizeCategory: #S } }

@VDM.viewType: #BASIC

define view entity I_CACorrespondenceVariantText
  as select from tfk070bt

  association [0..1] to I_Language as _Language on $projection.Language = _Language.Language

{
  key coprc as CACorrespondenceVariant,
      @Semantics.language
      @ObjectModel.foreignKey.association: '_Language'
  key spras as Language,

      @Semantics.text
      cotxt as CACorrespondenceVariantName,

      _Language

}
```
